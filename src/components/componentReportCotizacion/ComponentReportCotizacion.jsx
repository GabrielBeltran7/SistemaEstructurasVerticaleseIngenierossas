...
const ComponentReportCotizacion = () => {
  ...
  return (
    <div>
      ...
      <Table 
  columns={columns} 
  dataSource={reportecotizaciones} 
  rowClassName={() => style.rowWhiteBackground} 
/>
      </div>
      <Typography.Title level={4}>Total Cotizaciones: $ {totalAmount.toFixed(2)}</Typography.Title>
    </div>
  );
};
...
//   const columns = [
//     { title: "Nombre Empresa", dataIndex: "CompañiadelCliente", key: "CompañiadelCliente", ...getColumnSearchProps("CompañiadelCliente"), render: (text, record) => (<a href={`/AprobarCotizacion/${record.Numerocotizacion}`}>{text}</a>) },
